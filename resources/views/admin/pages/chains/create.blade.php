...
<div class="px-6 py-8 max-w-7xl mx-auto">
    <h3 class="text-3xl font-bold mb-6 text-white">Add Chain</h3>
    <div class="bg-crypto-card rounded-lg shadow border border-crypto-border p-6">
        <form method="POST" action="{{ route('admin.chains.store') }}" enctype="multipart/form-data">
            @csrf
            <div class="mb-6">
                <label class="block text-sm font-semibold text-gray-300 mb-2">Name</label>
                <input name="name" class="w-full px-4 py-2 bg-crypto-dark border border-crypto-border rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            ...
            <div class="mb-6">
                ...
                <input name="symbol" class="w-full px-4 py-2 bg-crypto-dark border border-crypto-border rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="ETH" required>
            ...
            <div class="flex items-center mb-6">
                <input class="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500" type="checkbox" name="is_active" value="1" checked>
                <label class="ml-2 text-sm font-medium text-gray-300">Active</label>
            </div>
            <div class="flex gap-3">
                <button class="px-6 py-2 bg-crypto-primary text-crypto-accent rounded-lg hover:bg-crypto-primary/80 transition font-medium">Save</button>
