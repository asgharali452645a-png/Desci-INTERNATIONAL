...
class WebController extends Controller
{
    ...
    {
        $popularCoins = Coin::where('is_active', true)
            ->where('show_in_trending', true)
    ...
    {
        $trendingCoins = Coin::with(['category'])
        ...
        $topGainers = Coin::with(['category'])
            ->where('is_active', true)
            ->where('show_in_gainers', true)
            ->orderByDesc('price_change')
            ->limit(10)
            ->get();

        $coins = Coin::with(['category', 'chains'])
            ->where('is_active', true)
            ->orderByDesc('marketcap')
            ->paginate(20);
        $coinTypes = CoinType::where('is_active', true)->get();
        $chains = Chain::where('is_active', true)->get();

        return view('web.pages.markets.index', compact(
            'trendingCoins',
            'topGainers',
            'coins',
            'coinTypes',
