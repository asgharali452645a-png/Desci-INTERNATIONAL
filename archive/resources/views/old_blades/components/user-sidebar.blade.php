    ...
    $navigationConfig = [
        'main' => [
            [
                ...
                'badge' => null,
                'active' => request()->routeIs('user.leaderboard'),
                'dropdown_key' => null
            ],
            [
                'title' => 'Contest',
                'href' => route('user.contest'),
                'icon' => 'M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.13 3.13 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.13 3.13 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.13-3.13 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.13-3.13z',
                'badge' => null,
                'active' => request()->routeIs('user.contest'),
                'dropdown_key' => null
            ],
            [
                'title' => 'Dashboard',
            ...
            [
                ...
                'badge' => null,
                'active' => request()->routeIs('user.orders'),
            ],
            [
                'title' => 'Performance Bonus',
                'href' => route('user.performance-bonus'),
                'icon' => 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
                'badge' => null,
                'active' => request()->routeIs('user.performance-bonus'),
            ],
            [
                'title' => 'Rewards Hub',
                'href' => route('user.rewards-hub'),
                'icon' => 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4',
                'badge' => null,
                'active' => request()->routeIs('user.rewards-hub'),
            ],
            [
                'title' => 'Account',
                        ...
                        @foreach($navigationConfig['main'] as $item)
                            <li>
                                @if(isset($item['children']))
                                    ...
                                    <div>
                                        <button
                                            @click="dropdowns.{{ $item['dropdown_key'] }} = !dropdowns.{{ $item['dropdown_key'] }}"
                                            ...
                                            <span x-show="!sidebarCollapsed"
                                                class="flex-1 text-left">{{ $item['title'] }}</span>
                                            @if(isset($item['badge']))
                                                <span x-show="!sidebarCollapsed"
                                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 ml-auto">{{ $item['badge'] }}</span>
                                            @endif
                                            <svg x-show="!sidebarCollapsed" class="ml-auto h-4 w-4 transition-transform"
                                                :class="{ 'rotate-90': dropdowns.{{ $item['dropdown_key'] }} }" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M9 5l7 7-7 7"></path>
                                            </svg>
                                        </button>
                                        <ul x-show="dropdowns.{{ $item['dropdown_key'] }} && !sidebarCollapsed" x-transition
                                            class="mt-1 px-2">
                                                ...
                                                <li>
                                                    <a href="{{ $child['href'] }}"
                                                        ...
                                                        @if(isset($child['badge']))
                                                            ...
                                                            <span
                                                                class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{{ $child['badge'] }}</span>
                                                        @else
                                                            {{ $child['title'] }}
                                                        @endif
                                                    </a>
...
<div class="lg:hidden">
    ...
    <div class="fixed top-16 left-0 right-0 z-40 bg-crypto-accent border-b border-gray-700">
        <div class="flex items-center justify-between px-4 py-3">
            <button @click="mobileExpanded = !mobileExpanded" class="p-2 text-white hover:bg-gray-700 rounded-md">
                        ...
                        <div>
                            <button @click="dropdowns.{{ $item['dropdown_key'] }} = !dropdowns.{{ $item['dropdown_key'] }}"
                                ...
                                <div class="flex items-center gap-x-3">
                                    ...
                                    @if(isset($item['badge']))
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">{{ $item['badge'] }}</span>
                                    @endif
                                </div>
                                <svg class="h-4 w-4 transition-transform"
                                    :class="{ 'rotate-180': dropdowns.{{ $item['dropdown_key'] }} }" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
